...
def barycentric_coordinates(x0, y0, x1, y1, x2, y2, x, y): #барицентрические координаты
...
def draw_triangle(x0, y0, x1, y1, x2, y2, image):
    #color = (random.randint(0,255),random.randint(0, 255), random.randint(0,255)) #рандомные значения для цвета зайца
    color = (0, 255, 0)
    pixel = image.load()
...
def main():
    ...
    for face in arr_f:
        ...
        # Рисуем треугольник
        draw_triangle(x0, y0, x1, y1, x2, y2, image)
    image.save('green_banny.png')
    image.show()
    return image
